...
- 1계층에서 사용되는 통신단위는 비트이며 1(on) / 0(off)
...
- 물리계층에서 받은 정보를 bridge, switch 징비를 통해 MAC 주소로 전달
- 물리 계층을 통해 송수진하는 데이터의 오류와 흐름을 관리
...
- 전송 계층은 패킷들의 전송이 유효한지 확읺고 전송 실패한 패킷을 다시 전송
- TCP, UDP 주로 사용
...
- connection-oriented 연결지향적: 목적지와 연결되면 그 이후에는 데이터만 보내면 됨
  - UDP는 비연결성이므로 데이터를 보낼 떄마다 목적지를 설정해야함
<br>
