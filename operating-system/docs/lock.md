...
- thread_A가 Lock을 풀기 위해 unlock() 을 호출해 ```flag == 0```으로 만들었을 때 thread_B는 무한루프(Spinlock)에서 빠져나올 수 있음
<br>
...
thread_A와 thread_B가 lock 메서드를 호출했지만 thread_A의 처리 속도가 조금 더 빨랐다고 가정한다.
...
- Lock이 걸려있지 않다면 new 값으로 갱신하고 이전 값인 0 반환
